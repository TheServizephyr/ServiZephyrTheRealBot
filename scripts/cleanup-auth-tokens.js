/**
 * Auth Tokens Cleanup Script
 *
 * Usage:
 *   node scripts/cleanup-auth-tokens.js
 *   DAYS=7 node scripts/cleanup-auth-tokens.js
 *   MODE=full node scripts/cleanup-auth-tokens.js
 *
 * Default mode:
 *   - stale: delete auth_tokens older than DAYS using:
 *     1) expiresAt (preferred)
 *     2) createdAt (fallback)
 *
 * Full mode:
 *   - full: delete all docs from auth_tokens
 *
...
async function run() {
    ...
    const collection = firestore.collection('auth_tokens');

    console.log('========================================');
    console.log('Auth Tokens Cleanup');
