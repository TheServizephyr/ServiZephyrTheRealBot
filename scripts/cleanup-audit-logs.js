 ...
 *   DAYS=7 node scripts/cleanup-audit-logs.js
 *   MODE=full node scripts/cleanup-audit-logs.js
 *
 * Default mode:
 *   - stale: delete audit_logs older than DAYS (default: 7)
 *     Timestamp fallback order:
 *     1) createdAt
 *     2) timestamp
 *
 * Full mode:
 *   - full: delete all docs from audit_logs collection
 *
...
async function run() {
    ...
    const collection = firestore.collection('audit_logs');

    console.log('========================================');
    console.log('Audit Logs Cleanup');
