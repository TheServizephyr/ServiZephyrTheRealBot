...
const mapRestaurantDataFromBootstrap = (bootstrapData) => {
    if (!bootstrapData || typeof bootstrapData !== 'object') return null;
    ...
    return {
        ...
        name: business.name || '',
        status: business.approvalStatus || null,
        logoUrl: business.logoUrl || '',
        bannerUrls: Array.isArray(business.bannerUrls) && business.bannerUrls.length > 0 ? business.bannerUrls : ['/order_banner.jpg'],
        ...
        gstCalculationMode: charges.gst?.mode || (charges.gst?.includedInPrice === false ? 'excluded' : 'included'),
...
const OrderPageInternal = ({ initialBootstrap = null, initialSearchParams = {} }) => {
    ...
    const [isTokenValid, setIsTokenValid] = useState(() => Boolean(initialBootstrap));
    ...
    const [restaurantData, setRestaurantData] = useState(() => mapRestaurantDataFromBootstrap(initialBootstrap) || getDefaultRestaurantData());
    const [tableStatus, setTableStatus] = useState(null);
    const [loyaltyPoints, setLoyaltyPoints] = useState(() => Number(initialBootstrap?.user?.customer?.loyaltyPoints || 0));
    ...
    const hasInitialRenderableShell = useMemo(() => {
        const hasName = Boolean(String(restaurantData?.name || '').trim());
        const hasCategories = Array.isArray(restaurantData?.customCategories) && restaurantData.customCategories.length > 0;
        const hasMenuItems = Boolean(restaurantData?.menu && Object.keys(restaurantData.menu).length > 0);
        return hasName && (hasCategories || hasMenuItems);
    }, [restaurantData]);
    ...
    if (loading && !hasInitialRenderableShell) {
    ...
    if (!isTokenValid && !hasInitialRenderableShell) {
        return <div className="min-h-screen bg-background flex items-center justify-center"><GoldenCoinSpinner /></div>;
    }
...
const OrderPage = ({ initialBootstrap = null, initialSearchParams = {} }) => {
    const [isPathCanonicalized, setIsPathCanonicalized] = useState(true);
