...
async function getRestaurantResult(firestore, merchantId) {
  ...
  const orders = orderDocs.map((doc) => normalizeOrder(doc)).sort((a, b) => new Date(b.orderDate || 0) - new Date(a.orderDate || 0));
...
export async function POST(req) {
  try {
    ...
    if (normalizedType === 'customer') {
      ...
      if (!data) {
        return NextResponse.json({ message: `No customer found for ID ${normalizedId}.` }, { status: 404 });
      }
    } else if (normalizedType === 'restaurant') {
      data = await getRestaurantResult(firestore, normalizedId);
      if (!data) {
        return NextResponse.json({ message: `No restaurant found for ID ${normalizedId}.` }, { status: 404 });
      }
    } else if (normalizedType === 'order') {
      data = await getOrderResult(firestore, normalizedId);
      if (!data) {
        return NextResponse.json({ message: `No order found for ID ${normalizedId}.` }, { status: 404 });
      }
    } else {
