...
export async function POST(request) {
    console.log("[Webhook WA Processor] POST request received.");
    try {
        ...
        if (currentSigningKey && nextSigningKey) {
            ...
            if (signature) {
               const isValid = await receiver.verify({ signature, body: rawBodyText });
               if (!isValid) {
                   console.error('[Webhook WA Processor] ❌ SECURITY: Invalid QStash signature. Rejecting request.');
                   return new NextResponse('Invalid QStash signature', { status: 403 });
               }
               console.log('[Webhook WA Processor] ✅ QStash signature verified.');
            } else {
               if (process.env.NODE_ENV === 'production') {
                   console.error('[Webhook WA Processor] ❌ SECURITY: Missing Upstash signature in production. Rejecting request.');
                   return new NextResponse('Missing Upstash signature', { status: 403 });
               } else {
                   console.warn('[Webhook WA Processor] ⚠️ Missing Upstash signature, but allowing in development.');
               }
            }
        } else {
            console.warn('[Webhook WA Processor] ⚠️ QSTASH_CURRENT_SIGNING_KEY not set. Skipping QStash signature verification.');
        }
