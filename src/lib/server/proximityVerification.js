...
export function verifyPublicIntakeProximity(businessData = {}, body = {}) {
    ...
    if (!businessCoordinates) {
        return { ok: false, code: 'LOCATION_NOT_CONFIGURED', message: 'Restaurant location is not configured. Please contact the restaurant.' };
    }
    ...
    if (latitude === null || longitude === null || accuracy === null) {
        return { ok: false, code: 'LOCATION_REQUIRED', message: 'Live location is required. Please allow precise location access and try again.' };
    }
    if (Math.abs(latitude) > 90 || Math.abs(longitude) > 180 || accuracy < 0 || accuracy > PUBLIC_INTAKE_RADIUS_METERS) {
        return { ok: false, code: 'LOCATION_ACCURACY_LOW', message: 'Your location accuracy must be within 50 meters. Move outdoors or enable precise location and try again.' };
    }
    ...
    if (distanceMeters > PUBLIC_INTAKE_RADIUS_METERS) {
        return {
            ok: false,
            code: 'OUTSIDE_LOCATION_RADIUS',
            message: `You must be within 50 meters of the restaurant. You are approximately ${Math.round(distanceMeters)} meters away.`,
