...
const LEGACY_ONLY_PAYMENT_METHODS = new Set(['online', 'razorpay', 'split_bill']);
const SUPPORTED_PAYMENT_METHODS = new Set(['cod', 'cash', 'counter', 'pay_at_counter', 'phonepe']);
...
async function createOrderNative({ req, body }) {
  ...
  try {
    ...
    const status = paymentMethod === 'phonepe' ? 'awaiting_payment' : 'pending';
    const paymentStatus = paymentMethod === 'phonepe' ? 'pending' : 'pending';
      ...
      statusHistory: [
        {
          status,
          timestamp: new Date(),
          notes: paymentMethod === 'phonepe' ? 'Waiting for payment confirmation' : 'Order created',
        },
      ],
    };
