...
async function getBusinessRef(firestore, restaurantId) {
  ...
  let businessRef = firestore.collection('restaurants').doc(safeRestaurantId);
  let businessSnap = await businessRef.get();
  if (businessSnap.exists) {
    return businessRef;
  }

  businessRef = firestore.collection('shops').doc(safeRestaurantId);
...
async function getOwnerTables(req) {
  ...
  if (!restaurantId || !tableId) {
    throw new HttpError(400, 'Restaurant ID and Table ID are required.');
  }
...
async function postOwnerTables(req, body = {}) {
  ...
  if (!tableId || !restaurantId || !paxCount || !tabName) {
    throw new HttpError(400, 'Table ID, Restaurant ID, pax count, and tab name are required.');
  }
